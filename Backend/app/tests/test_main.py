...
def test_ai_generate_testcases():
    token = get_admin_token()
    res = client.post("/ai/generate-testcases",
        json={"requirement": "user login"},
        headers={"Authorization": f"Bearer {token}"}
    )
    assert res.status_code == 200
    assert "generated_testcases" in res.json()
    assert len(res.json()["generated_testcases"]) > 0


def test_ai_analyze_defect():
    token = get_admin_token()
    res = client.post("/ai/analyze-defect",
        json={"description": "App crashes on login"},
        headers={"Authorization": f"Bearer {token}"}
    )
    assert res.status_code == 200
    assert "predicted_severity" in res.json()
    assert res.json()["predicted_severity"] == "Critical"


def test_ai_risk_analysis():
    token = get_admin_token()
    res = client.get("/ai/risk-analysis",
        headers={"Authorization": f"Bearer {token}"}
    )
    assert res.status_code == 200
...
def test_ai_test_summary():
    token = get_admin_token()
    res = client.get("/ai/test-summary",
        headers={"Authorization": f"Bearer {token}"}
    )
    assert res.status_code == 200
    assert "health_score" in res.json()import pytest
from fastapi.testclient import TestClient
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from app.main import app
